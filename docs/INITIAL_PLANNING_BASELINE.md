...
| 地图/定位/媒体 | 基础版真实实现 | 地图、定位、相机尚不存在；Android/HarmonyOS 已接系统相册，原图复制到应用沙箱并有引用保护清理 | 相册能力在基线扫描后随 Record Flow Phase 2 实现；其余差异原因未记录 |
| 城市碎片/时间轴 | 基础版核心闭环 | 已有模型、Repository、草稿/发布/编辑/删除、编辑器、时间轴、相册和详情代码闭环 | Record Flow Phase 2 于 2026-07-27 实现；设备体验仍需双端真机验收 |
