...
地点详情可按地点名称与城市临时查询高德地图 Web 服务返回的 POI 首图。图片 URL 不写入地点模型、MMKV 或备份包，不作为仓库内置摄影资产再分发；界面必须就近显示“图片来源：高德地图 POI”。请求失败、没有匹配图片或图片加载失败时继续使用代码生成的类别 fallback。地点文字的 `contentSource` 与图片来源分别展示，不能因为补充了在线图片而把内置介绍改称高德内容。
