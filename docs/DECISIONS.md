...
## ADR-035：在线地点查询页使用进程内 LRU + TTL，不持久化

- **状态**：已采用（2026-08-31）。
- **依据**：在线搜索、分类切换和详情补图会重复请求相同高德 POI 页；结果可重新获取、体积有界且会随时间变化，不属于地点事实，也不应写入 MMKV 或备份。
- **决策**：`CachingPlaceRemoteDataSource` 装饰真实 `PlaceRemoteDataSource`，按规范化关键词、城市、约百米位置网格、页码和页大小缓存成功页；进程内最多 32 页、10 分钟 TTL，访问命中刷新 LRU 顺序。失败、断网和 Unavailable 不缓存。
- **结果**：返回同一查询可减少网络等待和 API 配额消耗；进程结束、TTL 到期或 LRU 淘汰后正常重新请求。缓存不替代列表虚拟化、稳定 key 或平台图片字节缓存。

## ADR-034：共享层控制地点图片请求准入，平台层负责字节缓存与解码

- **状态**：已采用（2026-08-19）。
- **依据**：地点卡片由 commonMain 统一组合，但实际图片由双端 Render adapter 加载；把 Bitmap、磁盘路径或 Glide 对象引入共享业务层会破坏平台边界。
- **决策**：commonMain 只实现优先级、最多 3 并发、URL 去重、lease 生命周期和进程内指标；平台 adapter 继续负责下载、内存/磁盘缓存与目标尺寸解码。MMKV 只保存有界 URL 元数据。
- **结果**：双端获得一致的调度与 fallback 语义，Android 可复用现有 Glide 能力；HarmonyOS 的实际缓存表现仍需真机量化。

## ADR-033：地点远程图片使用有界可失效缓存，不进入地点事实或备份

- **状态**：已采用（2026-08-17）。
- **依据**：高德 POI 图片 URL 是运行时补充内容，可能失效，不应改变 `Place.visualRef`、地点内容来源或长期备份语义；Home/Explore 列表逐项联网会造成请求风暴和布局不稳定。
- **决策**：详情页缺少本地封面时按需查询，成功后写入 `cc_cache/places.photo_cache`；最多 100 条、30 天有效。Home/Explore 只读缓存；加载失败立即删除；备份明确排除该 Key。
- **结果**：所有地点卡片使用“本地封面 → 有效远程缓存 → 类别 fallback”的同一视觉规则。缓存清空、过期或离线不会影响地点数据及可浏览性。

## ADR-031：在线 POI 按需查询并选择性导入

- **状态**：已采用（2026-08-17）。
- **依据**：当前 Place catalog 有 500 条明确上限且使用 MMKV；高德 Web API 可以按城市、关键词或附近范围查询。Explore 只把用户明确选择的单个 POI 作为 `IMPORTED` 地点写入本地。
- **结果**：不批量缓存整座城市 POI；无网络仍可浏览 seed、用户地点和已经导入的地点。

## ADR-032：高德网络坐标只在 RemoteDataSource 边界转换

- **状态**：已采用（2026-08-17）。
- **依据**：Place 的持久坐标契约是 WGS-84，而高德国内地点与逆地理编码请求使用其地图坐标。共享高德数据源在请求前执行 WGS-84→GCJ-02、响应后执行 GCJ-02→WGS-84。
- **结果**：Repository、路线、距离、备份和领域模型不保存供应商坐标。

## ADR-029：一次性当前位置不持久化，用户地点坐标显式持久化

- **状态**：已采用（2026-08-17）。
- **依据**：当前 `LocationCapability` 只在用户主动操作后请求；`CurrentLocationRuntime` 仅保存进程内成功结果，Home/Explore/Map 复用它做距离与相机状态。`PlaceDraft.geoPoint` 则由用户手填或明确选择“使用当前位置”后随 Place V3 保存。
- **结果**：浏览位置不会被 MMKV 长期记录；用户创建地点的坐标属于地点内容，可跨重启和备份恢复。

## ADR-030：地点封面与城市碎片照片共享引用保护后再删除

- **状态**：已采用（2026-08-17）。
- **依据**：地点封面和 Capsule 图片都可能指向应用沙箱托管文件。`RepositoryPlaceMediaCleanup` 删除候选文件前读取 Place catalog、已发布 Capsule 和草稿；任一读取失败即停止删除。
- **结果**：替换、移除、放弃或删除地点封面不会误删仍被其他业务对象引用的文件。
...
## ADR-026：结束漫游保存有界回顾快照，不新增一级 Tab

状态：Accepted。

- 现有单条 `roaming.session` 只能支持当前/最后会话；为了让已结束漫游可长期回顾，新增有界 `roaming.history` catalog，并随备份协议 v10 导出/恢复。
- 归档保存结束当时的地点名称、顺序、到达时间/方式和轨迹摘要，避免地点后续改名/删除破坏历史可读性。
- 归档是幂等的；重启后遇到已结束会话会再次补归档，但不产生重复历史。
- Capsule 继续以既有 `roamingSessionId` 精确关联漫游，不为本功能改动 Capsule wire schema。
- 漫游入口位于想去/路线和 Record 内部的回顾页。正式一级导航仍为“探索 / 记录 / 我的”，真机闭环验收不自动构成增加 Tab 的决策。

依据：用户明确要求漫游与想去联动并产生可回顾城市碎片；现有代码已以会话开始时间作为 Capsule 关联 ID，且 `AGENTS.md` 明确规定基础阶段不新增漫游 Tab。

## ADR-027：轨迹全量落文件，地图只渲染有界显示点

状态：Accepted。

- 轨迹真实数据仍是 `files/tracks` 分片，不修改 MMKV 索引和 Track schema。
- 漫游地图复用已有双端 `CCAmapView`，在 provider-neutral State 中增加 WGS-84 `trackPoints`，不让 Store 依赖高德类型。
- 为限制 Kuikly wire JSON、原生 Overlay 重建和长时漫游的渲染成本，地图最多显示 500 个等距抽样点，必须保留首尾；该抽样不影响距离计算、归档和备份引用。
- 地图隐私同意与轨迹记录解耦：用户拒绝地图渲染或 SDK 不可用时，已授权的本地轨迹采样仍可继续。

依据：用户明确要求漫游全程记录并实时地图展示；现有 Track 设计已确认“点进文件、MMKV 只存索引”。

## ADR-029：真实路线计算与双端原生地图渲染分层

状态：Accepted。

- 共享层继续持有地点、路线请求、道路级规划结果、Store/Repository 和 provider-neutral 地图 State；不得暴露 Android/HarmonyOS 高德 SDK 类型。
- Android 与 HarmonyOS 继续由各自原生 `CCAmapView`/adapter 渲染底图、Marker 和规划结果折线。当前工程已经采用该边界，不进行“改成双端原生地图”的重复重写。
- 路线计算是数据能力，不是地图绘制能力。后续通过独立的路线规划 Repository/RemoteDataSource 或平台 Capability 获取真实道路分段、距离和预计时长，再把统一结果交给双端原生地图渲染。
- 两点经纬度直连只能用于距离估算或诊断，不得作为用户可执行的步行路线，也不得称为“最佳路线”。多地点顺序优化必须基于真实道路成本；未证明全局最优时只称“推荐顺序”。

依据：当前代码已经使用共享 `ExploreMapViewState`/`MapMarkerModel` 协议，并分别在 Android 与 HarmonyOS 原生高德 adapter 中渲染；用户明确指出直线折线会产生穿越西湖等不可执行结果。

## ADR-033：地点细分类新增 wire 值并保留旧宽泛类别只读兼容

状态：Accepted。

- Place schema 和 `category` 字段不变；新增稳定 wire 值、产品名称与 Emoji。
- `culture/food/nature` 继续可解码，避免旧 MMKV、备份和用户地点失效，但不再作为新建地点可选项。
- 内置 seed 通过 seedVersion v4 刷新为细分类；在线 POI 在高德数据源边界根据返回 type 映射，无法确认时保持 `other`，不根据页面文案伪造。
- 细分类继续映射到既有六种代码生成视觉 fallback，因此本次不扩张图片资产和清理协议。

依据：用户明确要求把咖啡、美食、博物馆、教堂和历史遗迹等分开；当前类别是持久化 wire 值并被路由、筛选、备份共同使用，必须保持旧值可读。

## ADR-034：路线推荐使用有界真实道路矩阵，固定首点且不宣称全局最优

状态：Accepted；代码与双端构建完成，真实 API/真机待验收。

- P0 对用户手动顺序的每一对相邻地点调用高德步行 API，拼接 API 返回的道路折线、距离与时长；任何一段失败即整体失败，不用直线补洞。
- P1 固定用户当前第一个地点，只支持 2–8 个地点；每个无向地点对请求一次真实步行路线，最多 28 次请求，再以最近邻 + 2-opt 生成推荐顺序。
- 推荐结果只有用户点击“采用推荐顺序”后才进入待保存 `orderedPlaceIds`；算法结果称为推荐，不称全局最佳。
- 规划结果不持久化，避免把会随路网变化的数据写入当前 LocalRoute v1；最终确认的地点 ID 顺序仍沿用既有协议。

依据：高德步行接口按起终点返回道路折线、距离和时长；当前路线最多 20 点，但全矩阵请求随地点数平方增长，因此对自动推荐设置 8 点上限，更多地点继续支持 P0。

## ADR-035：增量地点必须是独立 Lazy item，分页由真实视口驱动

状态：Accepted；共享代码与自动化完成，HarmonyOS 性能验收待完成。

- 长列表页面使用接收 `LazyListScope` 的固定 Header Scaffold；不得把全部业务行包进 LazyColumn 的单一 item/普通 Column。
- 本地点和在线地点分别使用 `local:<placeId>` 与 `online:<providerId>`，同时获得业务身份稳定和跨数据源 key 隔离。普通 Compose `key` 不能替代 Lazy item 身份与虚拟化。
- 下一页只在 `LazyListState` 的真实最后可见 key 进入在线末 3 项时触发；Loading、失败重试和末页提示使用独立、固定高度 Footer。
- Store 以查询代次丢弃旧上下文回调，以已请求页集合合并重复 Intent；分页失败保留旧结果并只通过显式 Footer 操作重试。
- Reducer 的 Append 必须保持旧列表为新列表完整前缀，只在尾部追加并按 provider ID 去重；分页期间不重排旧地点。

依据：2026-09-02 对 HarmonyOS 增量滚动抖动的代码排查确认，原列表只有一个真实 Lazy item，末尾普通 Column 节点可在未进入视口时触发连续分页，Append 会重新测量整段内容；仅添加 Compose `providerId` key 无法提供 Lazy 虚拟化和滚动锚定。

## ADR-036：漫游形成真实闭环后进入第四个一级 Tab

状态：Accepted；取代 ADR-026 中“不新增一级 Tab”的阶段性限制，其余历史快照与关联规则保持不变。

- AppShell 当前固定承载“探索 / 记录 / 漫游 / 我的”四个根内容；漫游不建立第二套底栏或独立原生根栈。
- 漫游一级页只读取真实活动会话、本地路线、想去、漫游历史和关联城市碎片；没有数据时显示诚实空态，不提前暴露后台轨迹等未完成能力。
- Map 继续是探索与漫游的内容能力，不单独成为一级 Tab。
