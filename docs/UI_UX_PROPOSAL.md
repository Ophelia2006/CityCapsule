...
> 状态说明：第 1 节是 2026-07-27 的历史 UI 审计快照，不代表当前实现。当前状态以 `CURRENT_STATE.md` 为准：Record Flow 首版与单一 AppShell 一级导航已经实现，Home、Explore、Profile 等产品化重构仍未完成。第 2 节起继续作为后续设计基线。

## 1. 当前 UI 问题报告

### 全局信息架构

截至 2026-07-27，项目还没有稳定的一级导航。Home 当时用“浏览地点 / 我的收藏 / 打开本地档案 / 打开设置”大按钮充当功能目录，用户看到的是工程模块而非“今天去哪里、去过哪里”。记录与回忆核心内容当时尚不存在，导致产品重心被地点 CRUD 和设置取代。该问题后来已由 Record Flow 首版和 P0-3A 单一 AppShell 部分解决。

当时页面之间虽有 typed route，但产品路径断裂：地点详情不能进入记录；Home 没有内容聚合；Profile 不能回看城市档案。当前地点详情到城市碎片、时间轴与回忆的代码闭环已经建立；Home 内容聚合和 Profile 产品化仍未完成。路由技术闭环不等于用户闭环。
...
截至 2026-07-27 尚无页面，因此当时无法评价现有视觉。当前 Timeline、Gallery、Capsule Editor/Detail 已形成首版代码闭环，但月份分组、内部 HorizontalPager、平板体验和真机视觉验收仍未完成。
