                ...
                state.items.none { it.capsule.imagePaths.isNotEmpty() } -> {
                    ...
                    items = state.items.flatMap { item ->
                        item.capsule.imagePaths.mapIndexed { index, path ->
                            GalleryPhoto(item.capsule, item.place, item.dateLabel, path, index)
                        }
                    },
...
private fun GalleryGrid(
    ...
    com.y.citycapsule.designsystem.component.PhotoGrid(items = items) { item ->
        Column {
            CapsulePhoto(
                path = item.path,
            ...
            AppCaptionText(item.place?.name ?: item.dateLabel)
        }
    }
}
