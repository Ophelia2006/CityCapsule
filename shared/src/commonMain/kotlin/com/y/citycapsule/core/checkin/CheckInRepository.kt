...
class CheckInRepository(private val storage: KeyValueStore) {
    ...
    fun add(session: Long, placeId: String, method: CheckInMethod, distance: Double?, wasWantTo: Boolean? = null, callback: StorageCallback<CheckInCatalog>) = prepare(session) { r -> if(r !is StorageResult.Success)return@prepare callback(failure()); val c=r.value; if(c.checkIns.any{it.placeId==placeId})return@prepare callback(StorageResult.Success(c)); val v=c.copy(checkIns=c.checkIns+CheckIn(placeId,DateTime.currentTimestamp(),method,distance,wasWantTo)); storage.put(AppStorageKeys.Roaming.CHECK_INS,v){callback(if(it is StorageResult.Success)StorageResult.Success(v) else failure())} }
