...
fun AppBottomNavigation(
    ...
    Column(modifier = modifier.fillMaxWidth().background(AppTheme.colors.surface)) {
        AppDivider()
        Row(
            modifier = Modifier.fillMaxWidth().height(AppTheme.dimensions.bottomNavigationHeight),
            verticalAlignment = Alignment.CenterVertically
        ) {
            items.forEach { item ->
                val selected = item.id == selectedId
                Column(
                    modifier = Modifier.weight(1f).clickable { onSelected(item.id) }
                        .padding(vertical = AppTheme.dimensions.spacingXs),
                    horizontalAlignment = Alignment.CenterHorizontally
                ) {
                    AppIcon(item.icon, item.label, tint = if (selected) AppTheme.colors.primary else AppTheme.colors.textSecondary)
